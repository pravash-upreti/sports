...
export default class ValentinesCup extends Vue {
  ...
  public async fetchData() {
    ...
    try {
      ...
      this.updateSelectedSport(SPORTS.VALENTINES_CUP);

      const response = await fetchSportData(SPORTS.VALENTINES_CUP, this.season);
  ...
  get title(): string {
    return `Valentines Cup - ${this.season}`;
  }

  get selectedSportSeason(): object {
    return {
      sport: SPORTS.VALENTINES_CUP,
      season: this.season
    };
  }
}
